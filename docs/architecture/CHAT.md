...
| **12** | ✅ | Arrow-key model selector modal (Ctrl+M). Textual ModalScreen with OptionList (up/down/Enter built-in). Loads instantly, fetches the model list in worker, populates list. Currently-active model marked ▶. Dismiss callback applies registry.set_active_model + refreshes subtitle. Reuses list_models + set_active_model — no duplication. 9 unit tests for construction + render paths + dismiss handling. | ~270 | `09e909471` | `30fb4a25` |
