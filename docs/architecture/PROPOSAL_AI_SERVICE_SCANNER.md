...
**Related:**
- [`PROPOSAL_EVENT_LISTENER.md`](PROPOSAL_EVENT_LISTENER.md) — loop/listener primitives this rides on (shipped 1.9.2)
...
1. **Live running-state inventory, not source-code analysis.** Those tools scan code at rest. This scans processes, sockets, scheduled tasks, plugin manifests, and held connections — what's *actually executing on this machine right now.* The empirica-outreach orphan-cron incident (1.9.2 work) is the canonical failure mode; no static analyzer would have caught it.
...
3. **Local-first, developer-workflow integrated.** No cloud telemetry, no SOC analyst persona. Sits next to `empirica status`, integrates with the loop/listener/notify infrastructure already shipped in 1.9.2. Audience is solo devs and small teams running AI-heavy stacks on their own machines.
...
- Held curl listeners from 1.9.2 work (what triggered this proposal)
- CronCreate jobs from days/weeks ago — pre-1.9.2 we had no way to inventory them
...
New `#services` panel below `#compliance` (mirrors the pattern shipped in 1.9.2):
...
- **ntfy:** new high-confidence finding → push notification (re-uses 1.9.2 dispatcher)
...
|---|---|---|---|
| **1** | 1.9.2 | One-shot `empirica scan` + scanner module + read-surface YAML + bundled corpus + Markdown report. Deterministic only — no AI judgment yet. | ~3 days |
| **2** | 1.9.2 | `--explain` AI judgment layer + `services-auditor` persona + cockpit `#services` panel + click-to-expand. | ~5 days |
| **3** | 1.9.2 | Biweekly loop + ntfy integration + history/diff verbs + corpus-refresh loop. | ~3 days |
