...
**Related:**
- [`PROPOSAL_EVENT_LISTENER.md`](PROPOSAL_EVENT_LISTENER.md) — loop/listener primitives this rides on (shipped 1.8.19)
...
1. **Live running-state inventory, not source-code analysis.** Those tools scan code at rest. This scans processes, sockets, scheduled tasks, plugin manifests, and held connections — what's *actually executing on this machine right now.* The empirica-outreach orphan-cron incident (1.8.19 work) is the canonical failure mode; no static analyzer would have caught it.
...
3. **Local-first, developer-workflow integrated.** No cloud telemetry, no SOC analyst persona. Sits next to `empirica status`, integrates with the loop/listener/notify infrastructure already shipped in 1.8.19. Audience is solo devs and small teams running AI-heavy stacks on their own machines.
...
- Held curl listeners from 1.8.19 work (what triggered this proposal)
- CronCreate jobs from days/weeks ago — pre-1.8.19 we had no way to inventory them
...
New `#services` panel below `#compliance` (mirrors the pattern shipped in 1.8.19):
...
- **ntfy:** new high-confidence finding → push notification (re-uses 1.8.19 dispatcher)
...
|---|---|---|---|
| **1** | 1.8.19 | One-shot `empirica scan` + scanner module + read-surface YAML + bundled corpus + Markdown report. Deterministic only — no AI judgment yet. | ~3 days |
