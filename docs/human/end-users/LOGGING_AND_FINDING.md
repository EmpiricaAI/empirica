...
Edges are typed: `evidence`, `raised_by`, `grounded_by`, `resolves`, `invalidates`, `sourced_from`, `caused_by`, `prevents`, `attached_to`, plus the bead-specific `tracks` / `owned_by` / `about` / `worked_by`.
