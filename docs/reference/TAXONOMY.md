...
| **Falsifier** | The observation that would REFUTE a belief, registered before the evidence that tests it. Names its parent — a finding, assumption, decision, dead-end, mistake or lesson — and one without a parent is refused. Unlike a claim it outlives its transaction, surfacing at every PREFLIGHT until adjudicated `tripped`, `survived` or `expired`; `survived` requires evidence that the population was observed. Exists for the failure a confidence gate cannot see: a true measurement asserted past the population it was taken over. | Dynamic (state transitions) | WARM (SQLite) |
