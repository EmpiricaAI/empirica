...
def add_lesson_parsers(subparsers):
    ...
    lesson_create = subparsers.add_parser(
        ...
        epilog=(
            ...
            "Re-using an existing (name, version) is REFUSED (ok: false, code: 'exists') — "
            "the refusal names both designed paths: bump `version` (+ --supersedes so the old "
            "one stops steering), or use a new name. This help previously documented "
            "replace-in-place; that described a handler bug that swallowed the refusal, not "
            "the contract.\n\n"
